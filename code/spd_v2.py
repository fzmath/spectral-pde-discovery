"""
Spectral PDE Discovery (SPD) Framework v2
==========================================
Improved sparse regression:
1. Spatiotemporal spectral derivatives
2. Ridge initialization + adaptive threshold STLSQ
3. Coefficient stability ranking for term selection
"""
import numpy as np
...
def adaptive_stlsq(Theta, target, n_terms=None, alpha=1e-5, max_iter=20):
    """Adaptive STLSQ: automatically select top-k terms by coefficient magnitude.
    If n_terms is None, use elbow method on sorted coefficients.
    """
    n_features = Theta.shape[1]
    I = np.eye(n_features)
    xi = np.linalg.lstsq(Theta.T @ Theta + alpha*I, Theta.T @ target, rcond=None)[0]
    ...
    sorted_idx = np.argsort(np.abs(xi))[::-1]
    sorted_mag = np.abs(xi[sorted_idx])
    
    if n_terms is None:
        # Elbow method: find where coefficient magnitude drops sharply
        if len(sorted_mag) > 1 and sorted_mag[0] > 0:
            ratios = sorted_mag[1:] / (sorted_mag[:-1] + 1e-12)
            # Find first big drop
            elbow = np.argmin(ratios) + 1
            n_terms = min(elbow, n_features)
            # Ensure at least 1 term
            n_terms = max(1, n_terms)
        else:
            n_terms = 1
    
    # Iterative refinement with top-k threshold
    for _ in range(max_iter):
        threshold = sorted_mag[n_terms-1] * 0.5 if n_terms <= len(sorted_mag) else 0
        small = np.abs(xi) < threshold
        xi[small] = 0
        big = ~small
        if np.sum(big) == 0:
            break
        xi[big] = np.linalg.lstsq(Theta[:, big], target, rcond=None)[0]
        # Re-sort and update n_terms if needed
        sorted_idx = np.argsort(np.abs(xi))[::-1]
        sorted_mag = np.abs(xi[sorted_idx])
    
    return xi, n_terms

def stability_selection(Theta, target, n_bootstraps=20, threshold=0.02, alpha=1e-5):
    """Stability selection: run STLSQ on bootstrapped samples, count term frequency."""
    n_samples, n_features = Theta.shape
    counts = np.zeros(n_features)
    
    for _ in range(n_bootstraps):
        idx = np.random.choice(n_samples, size=n_samples, replace=True)
        xi = stlsq(Theta[idx], target[idx], threshold=threshold, alpha=alpha)
        counts += (np.abs(xi) > 1e-6).astype(float)
    
    # Select terms appearing in >60% of bootstraps
    selected = counts > 0.6 * n_bootstraps
    if np.sum(selected) == 0:
        selected = counts > 0.3 * n_bootstraps
    
    # Refit on selected terms
    if np.sum(selected) > 0:
        xi_final = np.zeros(n_features)
        xi_final[selected] = np.linalg.lstsq(Theta[:, selected], target, rcond=None)[0]
    else:
        xi_final = np.zeros(n_features)
    
    return xi_final, counts

# ============================================================
# SPD pipeline v2
# ============================================================

def spd_pipeline_v2(u, x, t, true_terms, max_order=4, poly_order=2):
    """SPD v2: spatiotemporal spectral + adaptive STLSQ + stability check."""
    ...
    # Step 3: adaptive STLSQ
    xi, n_terms = adaptive_stlsq(Theta, target, alpha=1e-5)
    
    # Step 4: stability selection to refine
    xi_stab, counts = stability_selection(Theta, target, n_bootstraps=15, threshold=0.01)
    
    # Combine: use adaptive result, but only keep terms that are stable
    stable_mask = counts > 0.5 * 15
    xi_final = xi.copy()
    xi_final[~stable_mask] = 0
    
    # If stability killed everything, fall back to adaptive
    if np.sum(np.abs(xi_final) > 1e-6) == 0:
        xi_final = xi
    
    recovered = {feature_names[i]: float(xi_final[i]) for i in range(len(xi_final)) if abs(xi_final[i]) > 1e-6}
    return recovered, n_terms
...
def test_method(method_name, u, x, t, true_terms, max_order=4):
    ...
    elif method_name == 'SPD v2 (ours)':
        recovered, n_terms = spd_pipeline_v2(u, x, t, true_terms, max_order=max_order)
        tf, fp, exact, ce = evaluate_recovery(recovered, true_terms)
...
def main():
    ...
    out_path = Path(r"H:\2026科研\Spectral-PDE-Discovery\code\spd_v2_results.json")
    ...
    print("=" * 80)
    print(f"{'Dataset':<25} {'Noise':<10} {'FD+STLSQ':<15} {'Spec+STLSQ':<15} {'SPDv2(ours)':<15}")
    print("-" * 80)
    for ds in datasets:
